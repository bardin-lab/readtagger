...
TEST_BAM = 'dm6.bam'
TEMPLATE_ARGS = namedtuple('args', ['target', 'source', 'output_path', 'mate_sequence_tag'])


def test_mateoperations(datadir, tmpdir, mocker):  # noqa: D103
    out = tmpdir.join('out.bam')
    args = TEMPLATE_ARGS(target=datadir[TEST_BAM], source=datadir[TEST_BAM], output_path=out.strpath, mate_sequence_tag='MS')
    main(args)
